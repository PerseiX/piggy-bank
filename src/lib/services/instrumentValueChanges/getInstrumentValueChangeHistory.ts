...
export type GetInstrumentValueChangeHistoryArgs = {
...
export async function getInstrumentValueChangeHistory(
  ...
  if (ownershipResult.error) {
    throw new InstrumentValueChangesServiceError(
      "Failed to verify instrument ownership",
  ...
  const historyResult = await supabase
    ...
    .select(
      "id,instrument_id,before_value_grosze,after_value_grosze,created_at",
    )
  ...
  if (historyResult.error) {
    throw new InstrumentValueChangesServiceError(
      "Failed to load instrument value change history",
...
function mapValueChangeRowToDto(
  row: InstrumentValueChangeRow,
): ValueChangeDto {
