...
import type {
  ListWalletInstrumentsSortField,
  ListWalletInstrumentsSortOrder,
} from "../../validation/instruments"
import {
...
import {
  InstrumentRowForDto,
  mapInstrumentRecordToDto,
} from "./mappers"

type ListWalletInstrumentsArgs = {
...
export async function listWalletInstruments({
  ...
  if (walletResult.error) {
    throw new ListWalletInstrumentsServiceError(
  ...
  if (instrumentsResult.error) {
    throw new ListWalletInstrumentsServiceError(
      "Failed to load instruments for wallet",
