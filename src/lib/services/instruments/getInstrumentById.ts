...
import {
  type InstrumentRowForDto,
  mapInstrumentRowToDto,
} from "./mappers"

type InstrumentOwnershipRow = Pick<
  Database["public"]["Tables"]["instruments"]["Row"],
  "owner_id" | "deleted_at"
>

export type GetInstrumentByIdArgs = {
  supabase: SupabaseClient
  ownerId: string
  instrumentId: string
}

export type GetInstrumentByIdFailureReason =
  | "notFound"
  | "forbidden"
  | "softDeleted"
...
export async function getInstrumentById(
  args: GetInstrumentByIdArgs,
): Promise<GetInstrumentByIdResult> {
  ...
  const instrumentRow = (instrumentResult.data ??
    null) as InstrumentRowForDto | null
  ...
  const ownershipRow = (ownershipResult.data ??
    null) as InstrumentOwnershipRow | null
  ...
  throw new GetInstrumentServiceError(
    "Instrument state is inconsistent after lookup",
  )
}
