...
import {
  type InstrumentSoftDeleteRow,
  mapInstrumentSoftDeleteRowToDto,
} from "./mappers"

type InstrumentMetadataRow = Pick<
  Database["public"]["Tables"]["instruments"]["Row"],
  "id" | "owner_id" | "deleted_at"
>

type SoftDeleteInstrumentArgs = {
...
export async function softDeleteInstrument({
  ...
  if (metadataResult.error) {
    throw new InstrumentSoftDeleteFailedError(
      "Failed to load instrument metadata before soft delete",
      { cause: metadataResult.error },
    )
  }

  const instrumentRow = (metadataResult.data ??
    null) as InstrumentMetadataRow | null
  ...
  if (updateResult.error) {
    ...
    throw new InstrumentSoftDeleteFailedError(
      "Failed to soft delete instrument",
  ...
  if (!updatedRow.deleted_at) {
    throw new InstrumentSoftDeleteFailedError(
      "Soft delete succeeded but deleted_at timestamp is missing",
    )
  }
