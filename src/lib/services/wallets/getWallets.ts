...
type InstrumentRow = Pick<
  Instrument,
  "wallet_id" | "invested_money_grosze" | "current_value_grosze" | "goal_grosze"
>
...
export type GetWalletsForOwnerParams = {
...
export async function getWalletsForOwner({
  ...
  if (instrumentsError) {
    throw new GetWalletsServiceError(
      "Failed to load instruments for wallets",
      instrumentsError,
    )
  }
...
function groupInstrumentsByWallet(
...
function buildAggregates(instruments: InstrumentRow[]): WalletAggregatesDto {
  const targetSum = sumInstrumentField(instruments, (instrument) =>
    instrument.goal_grosze ?? 0,
  )
  const currentSum = sumInstrumentField(
    instruments,
    (instrument) => instrument.current_value_grosze,
  )
  const investedSum = sumInstrumentField(
    instruments,
    (instrument) => instrument.invested_money_grosze,
  )
...
function sumInstrumentField(
  instruments: InstrumentRow[],
  selector: (instrument: InstrumentRow) => number,
): number {
