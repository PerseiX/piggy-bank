...
type WalletDeleteResponseRow = Pick<Database["public"]["Tables"]["wallets"]["Row"], "id" | "deleted_at">;

interface SoftDeleteWalletParams {
...
export async function softDeleteWallet({
  ...
  const deletedWallet = (updateResult.data ?? null) as WalletDeleteResponseRow | null;

  if (!deletedWallet || !deletedWallet.deleted_at) {
    throw new SoftDeleteWalletServiceError("Soft delete wallet response missing expected data");
  }
...
function handleUpdateError(error: PostgrestError, walletId: string, ownerId: string): never {
