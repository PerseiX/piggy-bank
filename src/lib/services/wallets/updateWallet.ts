...
import type { UpdateWalletCommand, Wallet, WalletUpdate, WalletUpdatedDto } from "../../../types";

type WalletRow = Pick<
  Database["public"]["Tables"]["wallets"]["Row"],
  "id" | "owner_id" | "name" | "description" | "deleted_at" | "created_at" | "updated_at"
>;
type WalletResponseRow = Pick<WalletRow, "id" | "name" | "description" | "created_at" | "updated_at">;
...
const WALLET_METADATA_COLUMNS = "id,owner_id,name,description,deleted_at,created_at,updated_at";
...
export class WalletUpdateNameConflictError extends Error {
  ...
  constructor(
    ...
    super(`Wallet "${walletName}" already exists for this owner (while updating "${walletId}")`);
...
export async function updateWallet({
  ...
  const walletResult = await supabase.from("wallets").select(WALLET_METADATA_COLUMNS).eq("id", walletId).maybeSingle();
  ...
  if (updateResult.error) {
    if (isUniqueConstraintViolation(updateResult.error)) {
      throw new WalletUpdateNameConflictError(walletId, updates.name ?? walletRow.name);
    }
